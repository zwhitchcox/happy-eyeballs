// borrowed from node-fetch
import http from 'http';
import zlib from 'zlib';
import {once} from 'events';
import Busboy from 'busboy';

type MockableServer = http.Server & {
	nextResponseHandler?: MockHandler;
}

type MockHandler = (res: http.ServerResponse) => any;

export default class TestServer {
	server: MockableServer;
	hostname: string;
	...
	mockResponse(responseHandler: MockHandler) {
		this.server.nextResponseHandler = responseHandler;
		return `http://${this.hostname}:${this.port}/mocked`;
	}

	router(this: MockableServer, request: http.IncomingMessage, res: http.ServerResponse) {
		const p = request.url;
